...
DEST_DIR="$pkgdir/usr/share/themes"
...
OPTIONS:
  ...
  -l, --libadwaita        Copy installed gtk-4.0 theme to config folder for all libadwaita app use this theme
...
uninstall_link() {
    rm -rf "/usr/share/themes/tokyonight-gtk-theme-Dark/gtk-4.0/"{assets,windows-assets,gtk.css,gtk-dark.css} 2>/dev/null
}
...
link_libadwaita() {
    ...
    cp -raf "${THEME_DIR}/gtk-4.0/gtk.css" "$pkgdir/etc/skel/.config/gtk-4.0/gtk.css" 2>/dev/null
    cp -raf "${THEME_DIR}/gtk-4.0/gtk-dark.css" "$pkgdir/etc/skel/.config/gtk-4.0/gtk-dark.css" 2>/dev/null
}
...
install_theme() {
	...
	sed -i "s|.*menu-opacity=.*|menu-opacity=0|" "$pkgdir/etc/skel/.config/xfce4/panel/whiskermenu"*".rc" 2>/dev/null
    fi
